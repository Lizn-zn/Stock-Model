...
def label_data(path, nb_class=3, store_path="data\\label_data"):
    ...
    for i in range(df.shape[1] - 1):
        ...
        '''
        there are two methods to label data,
        the first is sort the stock and label those stock in order
        the second is label stock by rate of return
        '''
        ...
        index = np.where(ratio >= 1.1)
        df.iloc[index[0], i] = 1
        index = np.where(ratio < 1.1)
        df.iloc[index[0], i] = 0

    df.drop(df.columns[-1], axis=1, inplace=True)
    df.to_csv(store_path + "\\label_1.csv")
